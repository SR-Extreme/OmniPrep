...
import {
    ...
    listAdminDsaQuestions,
    publishAdminDsaQuestion,
} from '@/lib/api/admin';
...
export default function AdminDsaQuestionsPage() {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            try {
                const result = await listAdminDsaQuestions(accessToken as string, {
                    status,
                    ...appliedFilters,
                    page,
                    limit: PAGE_SIZE,
    ...
    function setStatus(next: QuestionListStatus) {
        router.replace(`/admin/questions/dsa?status=${next}`);
    }
    ...
    async function handlePublish(question: AdminQuestionListItem) {
        ...
        try {
            await publishAdminDsaQuestion(accessToken, question.id, {
                isPublished: true,
            });
    ...
    async function handleDelete(question: AdminQuestionListItem) {
        ...
        try {
            await deleteAdminDsaQuestion(accessToken, question.id);
            setQuestions((current) => current.filter((row) => row.id !== question.id));
        } catch (err) {
            setError(
            ...
            <div className="grid gap-6 lg:grid-cols-[240px_1fr]">
                ...
                <section className="space-y-4">
                    <AdminPageHeader
                        ...
                        description="Browse, edit, publish, or delete DSA interview questions."
                    />
                    ...
                    {isLoading ? (
                        ...
                        <AdminEmptyState message={`No ${status} DSA questions yet.`} />
                    ) : (
                            ...
                            <QuestionListCard
                                ...
                                onEdit={
                                    status === 'draft'
                                        ? (q) =>
                                              router.push(
                                                  `/admin/create/dsa?id=${encodeURIComponent(q.id)}`,
                                              )
                                        : undefined
                                }
                                onPublish={
