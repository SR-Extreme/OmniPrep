...
function buildSystemDesignEvaluationSystemPrompt(
    ...
    return [
        ...
        'In feedback, explicitly assess how the candidate answered the follow-up questions — depth, correctness, trade-off reasoning, and whether answers addressed the probes.',
        'Factor follow-up answer quality into metricScores, strengths, weaknesses, and suggestions where relevant.',
        ...
        'feedback: narrative summary that covers both the initial design and the follow-up answers.',
        'suggestions: actionable improvements.',
    ].join(' ');
}
...
function buildBehavioralQuestionSystemPrompt(input: GenerateBehavioralQuestionInput): string {
    return [
        ...
        'It is NOT necessary to always ask only follow-ups after a main question in a given round/phase — mix fresh main questions with follow-ups as a real interviewer would.',
        'Prefer a new main question when the prior topic is sufficiently covered or when remaining questions in the phase should cover different themes.',
